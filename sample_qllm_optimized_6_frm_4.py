...
def save_checkpoint(model, optimizer, epoch, dataset_index, total_datasets, output_dir):
    """Save model checkpoint with comprehensive information"""
    checkpoint = {
        'epoch': epoch,
        'dataset_progress': f"{dataset_index + 1}/{total_datasets}",
    ...
    torch.save(checkpoint, path / f'checkpoint_epoch_{epoch}_dataset_{dataset_index}.pt')
    print(f"\nSaved checkpoint: {path / f'checkpoint_epoch_{epoch}_dataset_{dataset_index}.pt'}")
...
def get_default_datasets():
    """Return default dataset configurations"""
    return [
        {
            'name': 'wikitext',
        ...
        {
            'name': 'oscar',  # Replace bookcorpus with oscar
...
def get_default_config():
    ...
    return {
        ...
        'output_dir': 'quantum_checkpoints_6',
...
def run_from_cli():
    ...
    parser.add_argument('--output_dir', type=str, default='quantum_checkpoints_6',
                      help='Directory for saving checkpoints (default: quantum_checkpoints)')
    
    # Add dataset-related arguments
    parser.add_argument('--datasets', type=str, nargs='+',
                      help='List of dataset names to train on (optional, defaults provided)')
    parser.add_argument('--dataset_configs', type=str, nargs='+',
                      help='Corresponding dataset configurations (optional)')
    parser.add_argument('--dataset_max_samples', type=int, nargs='+',
                      help='Maximum samples to use from each dataset (optional)')
    ...
    # Only override datasets if explicitly provided
    if args.datasets:
        config['datasets'] = []
        for i, dataset_name in enumerate(args.datasets):
            dataset_config = {
                'name': dataset_name,
                'max_samples': args.dataset_max_samples[i] if args.dataset_max_samples and i < len(args.dataset_max_samples) else None,
                'config': args.dataset_configs[i] if args.dataset_configs and i < len(args.dataset_configs) else None,
                'split': 'train',
                'save_checkpoint': True
            }
            config['datasets'].append(dataset_config)
    else:
        print("\nUsing default datasets configuration:")
        for dataset in config['datasets']:
            print(f"- {dataset['name']} (max_samples: {dataset['max_samples']})")
...
def train_model_optimized(model, config):
    """Optimized training loop with support for multiple datasets"""
    ...
    if not isinstance(config['datasets'], list):
        raise ValueError("config['datasets'] must be a list of dataset configurations")
        ...
        for dataset_config in config['datasets']:
            ...
            try:
                ...
                def process_text(example):
                    if text_key == 'dialog':
                        # For daily_dialog format
                        return {'text': ' '.join(example[text_key])}
                    elif text_key == 'utterances':
                        # For conv_ai_2 format
                        return {'text': ' '.join(example[text_key])}
                    elif text_key == 'utterance':
                        # For empathetic_dialogues format
                        return {'text': example[text_key]}
                    elif text_key == 'previous_utterance':
                        # For blended_skill_talk format
                        utterances = [
                            example['previous_utterance'],
                            example.get('free_messages', ''),
                            example.get('guided_messages', '')
                        ]
                        return {'text': ' '.join(filter(None, utterances))}
                    else:
                        # Default handling
                        return {'text': example.get(text_key, '')}
                ...
                # Process batches from this dataset
                for batch in tqdm(train_loader, desc=f"Epoch {epoch+1} - {dataset_config['name']}"):
                    input_ids = batch['input_ids'].to(device)
                ...
                # Optional: Save checkpoint after each dataset
                if dataset_config.get('save_checkpoint', False):
                    save_checkpoint(model, optimizer, epoch, 
                                 f"{config['output_dir']}/{dataset_config['name']}_epoch_{epoch}")
                
            except Exception as e:
                print(f"Error processing dataset {dataset_config['name']}: {str(e)}")
                continue
        
        # Calculate and log average loss across all datasets
        avg_loss = total_loss / total_batches if total_batches > 0 else float('inf')
        print(f"\nEpoch {epoch+1} completed. Average Loss across all datasets: {avg_loss:.4f}")
