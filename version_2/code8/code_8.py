...
def train_model(model: nn.Module, dataset: torch.Tensor, args: argparse.Namespace) -> Tuple[nn.Module, Dict]:
    ...
    # Configure device-specific settings with defaults
    default_batch_size = getattr(args, 'batch_size', 32)
    
    if device == "mps":
        # MPS-specific settings
        batch_size = min(32, default_batch_size)
        gradient_accumulation_steps = max(1, default_batch_size // batch_size)
    else:
    ...
    device_configs = {
        'cuda': {
            'batch_size': min(4, default_batch_size),
            'grad_accum_steps': 4,
            'chunk_size': 50,
            'use_amp': True,
        ...
        'mps': {
            'batch_size': min(1, default_batch_size),
            'grad_accum_steps': 16,
            'chunk_size': 25,
            'use_amp': False,
        ...
        'cpu': {
            'batch_size': min(2, default_batch_size),
            'grad_accum_steps': 8,
            'chunk_size': 40,
            'use_amp': False,
    ...
    # Set attention chunk size for model
    for layer in model.layers:
        layer['attention'].max_chunk_size = config['attention_chunk_size']
    
    # Initialize AMP scaler for CUDA
    scaler = torch.cuda.amp.GradScaler() if use_amp else None
    
    # Optimizer with device-specific parameters
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=2e-5,
        betas=(0.9, 0.999),
        eps=1e-8 if device != 'mps' else 1e-4,  # Higher eps for MPS stability
        weight_decay=0.02
    )
    ...
    # Prefetch next batch (only for CUDA)
    def prefetch_batch(data, idx, batch_size):
        if device == 'cuda':
            next_idx = idx + batch_size
            if next_idx < len(data):
                next_batch = data[idx:next_idx].to(device, non_blocking=True)
    ...
    for epoch in range(args.epochs):
        ...
        chunk_pbar = tqdm(range(num_chunks), desc=f"Processing chunks", unit="chunk")
        ...
        for chunk_idx in chunk_pbar:
            ...
            batch_pbar = tqdm(range(0, len(chunk_data), batch_size), 
                            desc=f"Processing batches", leave=False)
            
            prefetched_batch = None
            for i in batch_pbar:
                try:
                    # Use prefetched batch or load new batch
                    if prefetched_batch is not None:
                        input_ids = prefetched_batch
                    else:
                        batch_end = min(i + batch_size, len(chunk_data))
                        input_ids = chunk_data[i:batch_end].to(device)
                    
                    # Prefetch next batch
                    prefetched_batch = prefetch_batch(chunk_data, i + batch_size, batch_size)
